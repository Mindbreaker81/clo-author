...
|-------|-------------|---------|
| **Verifier** | submission-package and reporting audit | Pass/fail (binary) |
...
| Literature search | Search one database at a time | Spawn agents for journals, PubMed, Cochrane, and registries |
...
The `/talk` skill dispatches the Storyteller (creator) and storyteller-critic (reviewer) to generate presentations in 4 formats (full-length / `job-market` compatibility mode, seminar, short, lightning). Supports both Beamer (output to `paper/talks/`) and Quarto RevealJS (output to `paper/quarto/` via `--quarto` flag). All content derives from `paper/main.tex`.
